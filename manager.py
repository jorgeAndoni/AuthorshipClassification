...
if __name__ == '__main__':
    ...
    #arguments = sys.argv
    #model = arguments[1] #1
    #use_stops = arguments[2] #2
    #word_selection = arguments[3] #3
    #folder_results = arguments[4] #4
    ...
    print('Stopwords?: ',use_stops)
    print('Word selection: ',word_selection)
    print('Output file: ',folder_results)
    percs = [str(val) + '%' for val in limiars]
    percs.insert(0, '0%')
    all_res = []
    for size in text_sizes:
        logger.info('Working with texts of ' + str(size) + ' words')
        obj = AutorshipAnalysis(logger=logger,text_partition=size, limiar=limiars, remove_stops=use_stops, embedding_model=model, feature_selection=word_selection , output_file=folder_results)
        scores = obj.analysis()
    ...
    path = model + '_' + word_selection + '.csv'
